...
class ValidatorDB:
    ...
    def _init_db(self):
        ...
        # Create indexes
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_validation_fetched ON validation_cache(fetched_at)"
        )
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_weights_updated ON weights_cache(updated_at)"
        )
