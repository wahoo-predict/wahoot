...
def get_active_uids(metagraph: "bt.metagraph.Metagraph") -> List[int]:
    ...
    return [uid for uid in range(metagraph.n) if metagraph.axons[uid].ip != "0.0.0.0"]


def get_weights_from_api(
    api_base_url: str, validator_db: Optional[ValidatorDB] = None
) -> Dict[str, float]:
    """
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            weights = {
                w["miner_id"]: w["weight"] for w in weights_data.get("weights", [])
            }
        ...
        if validator_db:
            ...
            if cached_weights:
                bt.logging.info(
                    f"Using cached weights from backup database ({len(cached_weights)} miners)"
                )
...
def get_wahoo_validation_data(
    ...
    for i in range(0, len(hotkeys), batch_size):
        ...
        try:
            ...
            params = {"hotkeys": ",".join(batch_hotkeys)}
            ...
            response = httpx.get(
                f"{WAHOO_API_URL}/api/v2/users/validation", params=params, timeout=30.0
            )
            ...
            bt.logging.warning(
                f"HTTP error getting WAHOO validation data for batch {i//batch_size + 1}: {e}"
            )
            ...
            bt.logging.warning(
                f"Failed to get WAHOO validation data for batch {i//batch_size + 1}: {e}"
            )
            # Try to use cached data if available
            if validator_db:
                for hotkey in batch_hotkeys:
...
def main():
    ...
    try:
        while True:
            ...
            if validation_data:
                bt.logging.info(
                    f"Retrieved validation data for {len(validation_data)} miners from WAHOO API"
                )
            ...
            if rewards.sum() > 0:
                try:
                    ...
                    bt.logging.info(
                        f"Updated on-chain weights for {len(active_uids)} miners"
                    )
                except Exception as e:
