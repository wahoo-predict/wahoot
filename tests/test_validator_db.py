...
def test_cleanup_old_cache(validator_db: ValidatorDB, db_path: Path):
    ...
    all_data = validator_db.get_cached_validation_data(["old_miner", "new_miner"], max_age_days=30)
    assert len(all_data) == 2
    
    deleted = validator_db.cleanup_old_cache(max_age_days=7)
    assert deleted >= 1
    
    remaining = validator_db.get_cached_validation_data(["old_miner", "new_miner"], max_age_days=30)
