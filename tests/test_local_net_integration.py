...
class TestLocalNetIntegration:
    ...
    def test_1_miner_responds_with_valid_synapse(
        self, mock_dendrite, mock_metagraph
    ):
    ...
    def test_1_communication_within_timeout(
        self, mock_dendrite, mock_metagraph
    ):
    ...
    def test_2_main_loop_all_9_steps(
        ...
        assert mock_build_uid_to_hotkey.called, "Step 3: build_uid_to_hotkey should be called"
        assert mock_get_validation_data.called, "Step 4: get_wahoo_validation_data should be called"
    ...
    def test_3_ema_scores_persist(self, tmp_path):
        ...
        assert "5FHneW46xGXgs5mUiveU4sbTyGBzmstUspZC92UhjJM694ty" in loaded, "Should contain saved hotkey"
    ...
    def test_4_ema_volume_scorer_computes_weights(self):
        ...
        records = [
            ValidationRecord(
                hotkey="hk1",
                performance=PerformanceMetrics(total_volume_usd=10000.0, realized_profit_usd=500.0),
            ),
            ValidationRecord(
                hotkey="hk2",
                performance=PerformanceMetrics(total_volume_usd=5000.0, realized_profit_usd=200.0),
            ),
        ]
    ...
    def test_5_handles_empty_data(self, mock_wallet, mock_subtensor, mock_dendrite, mock_metagraph, test_config):
    ...
    def test_6_end_to_end_multiple_iterations(
        ...
        mock_responses = [
            ...
            WAHOOPredict(event_id="test_event_123", prob_yes=0.6, prob_no=0.4, confidence=0.7),
            WAHOOPredict(event_id="test_event_123", prob_yes=0.5, prob_no=0.5, confidence=0.6),
            WAHOOPredict(event_id="test_event_123", prob_yes=0.8, prob_no=0.2, confidence=0.9),
        ]
        mock_dendrite.query.return_value = mock_responses
        ...
        assert mock_set_weights.call_count == iterations, f"Should set weights {iterations} times"
        assert mock_reward.call_count == iterations, f"Should compute rewards {iterations} times"
