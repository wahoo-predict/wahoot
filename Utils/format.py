        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.ClassDef):
                ...
                if not class_name[0].isupper():
                    issues.append(
                        f"Class '{class_name}' should start with uppercase (CamelCase)"
                    )
                elif "_" in class_name:
                    issues.append(
                        f"Class '{class_name}' should use CamelCase, not snake_case"
                    )
...
def format_file(
    file_path: Path, check_only: bool = False, verbose: bool = True
) -> bool:
    """
...
def main():
    ...
    parser.add_argument(
        "--all", action="store_true", help="Format all Python files in the project"
    )
    parser.add_argument(
        "--verbose",
        action="store_true",
