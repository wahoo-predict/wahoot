...
def check_processes():
    ...
    # Check validator
    try:
        validator_count = int(subprocess.check_output(
            ["pgrep", "-f", "python.*validator"], 
            stderr=subprocess.DEVNULL
        ).decode().strip().count('\n') + 1)
        results['validator_running'] = validator_count > 0
        print(f"{GREEN if results['validator_running'] else RED}{'✓' if results['validator_running'] else '✗'}{NC} Validator running: {validator_count} process(es)")
    except:
    ...
    # Check miners
    try:
        miner_count = int(subprocess.check_output(
            ["pgrep", "-f", "python.*miner"], 
            stderr=subprocess.DEVNULL
        ).decode().strip().count('\n') + 1)
        results['miners_running'] = miner_count >= 3
        print(f"{GREEN if results['miners_running'] else RED}{'✓' if results['miners_running'] else '✗'}{NC} Miners running: {miner_count} (expected 3+)")
    except:
...
def check_logs():
    ...
    if "Queried" in log_content and "miners" in log_content:
        ...
        if matches:
            ...
            print(f"{GREEN if results['miners_queried'] else YELLOW}{'✓' if results['miners_queried'] else '⚠'}{NC} Validator queried {queried_count} miners")
        else:
    ...
    print(f"{GREEN if iteration_count > 0 else YELLOW}{'✓' if iteration_count > 0 else '⚠'}{NC} Found {iteration_count} loop iterations")
...
def check_database():
    ...
    try:
        ...
        print(f"{GREEN if score_count > 0 else YELLOW}{'✓' if score_count > 0 else '⚠'}{NC} Scoring runs: {score_count}")
        ...
        print(f"{GREEN if unique_miners >= 3 else YELLOW}{'✓' if unique_miners >= 3 else '⚠'}{NC} Unique miners in DB: {unique_miners} (expected 3+)")
...
def check_weight_setting():
    ...
    if "✓✓✓ WEIGHTS SET SUCCESSFULLY" in log_content:
        ...
        if "too soon to commit weights" in log_content:
            ...
            print(f"   This is normal on localnet - weights will be set when tempo allows")
