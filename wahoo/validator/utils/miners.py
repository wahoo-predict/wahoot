...
def get_hotkey_for_uid(
    uid: int, uid_to_hotkey: Dict[int, str], metagraph: Optional[Any] = None
) -> Optional[str]:
    """
    Get hotkey for a specific UID with fallback and validation.
    
    This is a convenience function that:
    1. First checks uid_to_hotkey mapping (preferred)
    2. Falls back to metagraph.hotkeys[uid] if mapping not available
    3. Validates the hotkey before returning
    
    Args:
        uid: Miner UID
        uid_to_hotkey: Pre-built UID-to-hotkey mapping
        metagraph: Optional metagraph object for fallback lookup
        
    Returns:
        Optional[str]: Hotkey string if valid, None otherwise
    """
    # Try mapping first
    ...
    # Fallback to metagraph if provided
    ...
    logger.debug(f"Could not get valid hotkey for UID {uid}")
    return None
