...
def install_dependencies(venv_path: Optional[Path] = None) -> bool:
    ...
    if use_uv:
        ...
        try:
            ...
            if pyproject_file.exists():
                ...
                print(f"ERROR: Neither pyproject.toml nor requirements.txt found")
                return False
            ...
            print(f"ERROR: Failed to install dependencies with uv:")
            print(e.stderr)
            print("\nFalling back to pip...")
    ...
    if not use_uv:
        ...
        if needs_sudo and not is_root:
            print("\n" + "=" * 70)
            print("ERROR: Missing dependencies require installation with elevated privileges.")
            print("Please run one of the following:")
            if pyproject_file.exists():
                print(f"  1. Activate your virtual environment and run: uv pip install -e .")
                print(f"  2. Or with pip: pip install -e .")
            else:
                print(f"  1. Activate your virtual environment and run: pip install -r requirements.txt")
            print(f"  2. Run as root/admin: sudo python {' '.join(sys.argv)}")
            print("=" * 70)
            return False
...
def upgrade_database() -> bool:
    try:
        ...
        try:
            ...
            if head_rev is None:
                print("No Alembic migrations found. Database initialized with schema.sql")
                print("To create migrations, run: alembic revision --autogenerate -m 'Initial migration'")
                return True
