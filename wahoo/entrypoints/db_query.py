...
def connect_db():
    ...
    if not db_path.exists():
        ...
        print(f"   Run the validator first to create the database.")
        sys.exit(1)
...
def list_miners():
    ...
    print("=" * 100)
    print(f"{'Hotkey':<50} {'UID':<6} {'First Seen':<20} {'Last Seen':<20} {'Axon IP':<15}")
    print("-" * 100)
    for hotkey, uid, last_seen, first_seen, axon_ip in rows:
        ...
        print(f"{hotkey:<50} {uid_str:<6} {first_seen_str:<20} {last_seen_str:<20} {axon_ip_str:<15}")
...
def show_scores(limit: int = 20):
    ...
    for ts, hotkey, score, reason, uid in rows:
        ...
        reason_str = (reason[:17] + "...") if reason and len(reason) > 20 else (reason or "")
        print(f"{ts_str:<20} {hotkey:<50} {uid_str:<6} {score_str:<12} {reason_str:<20}")
...
def show_performance(hotkey: Optional[str] = None, limit: int = 10):
    ...
    print("=" * 140)
    print(f"{'Timestamp':<20} {'Hotkey':<50} {'Volume USD':<15} {'Trades':<8} {'Profit USD':<15} {'Win Rate':<10} {'Activity':<10}")
    print("-" * 140)
    for ts, hk, volume, trades, profit, win_rate, activity in rows:
        ...
        print(f"{ts_str:<20} {hk:<50} {volume_str:<15} {trades_str:<8} {profit_str:<15} {win_rate_str:<10} {activity_str:<10}")
...
def show_volume():
    ...
    print("=" * 110)
    print(f"{'Hotkey':<50} {'UID':<6} {'Volume USD':<15} {'Trades':<10} {'Last Updated':<20}")
    print("-" * 110)
    ...
    for hotkey, volume, trades, ts, uid in rows:
        ...
        print(f"{hotkey:<50} {uid_str:<6} {volume_str:<15} {trades_str:<10} {ts_str:<20}")
...
def show_miner_details(hotkey: str):
    ...
    print("=" * 80)
    print(f"UID:              {miner[1] if miner[1] else 'N/A'}")
    ...
    if latest_score:
        print(f"\nLatest EMA Score:   {latest_score[0]:.6f}" if latest_score[0] else "Latest EMA Score:   N/A")
    ...
    if latest_perf:
        print(f"\nLatest Performance:")
        print(f"  Volume USD:       ${latest_perf[0]:,.2f}" if latest_perf[0] else "  Volume USD:       N/A")
        print(f"  Trade Count:      {latest_perf[1]}")
        print(f"  Profit USD:       ${latest_perf[2]:,.2f}" if latest_perf[2] else "  Profit USD:       N/A")
        print(f"  Win Rate:         {latest_perf[3]*100:.1f}%" if latest_perf[3] else "  Win Rate:         N/A")
        print(f"  Activity Score:   {latest_perf[4]:.4f}" if latest_perf[4] else "  Activity Score:   N/A")
...
def main():
    ...
    scores_parser.add_argument("--limit", type=int, default=20, help="Number of scores to show")
    
    # Latest scores command
    subparsers.add_parser("latest-scores", help="Show latest score for each miner")
    
    # Performance command
    perf_parser = subparsers.add_parser("performance", help="Show performance snapshots")
    perf_parser.add_argument("--hotkey", type=str, help="Filter by specific miner hotkey")
    perf_parser.add_argument("--limit", type=int, default=10, help="Number of snapshots to show")
    
        # Volume command
    subparsers.add_parser("volume", help="Show prediction volume and trade counts for all miners")
    
# Miner details command
    miner_parser = subparsers.add_parser("miner", help="Show detailed info for a specific miner")
